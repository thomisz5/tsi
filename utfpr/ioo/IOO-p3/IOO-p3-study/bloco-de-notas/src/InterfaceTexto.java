...
public class InterfaceTexto {
    ...
    private void adicionar() {
        ...
        if (nota.eValida())
            ...
            sln("Os seguintes erros foram encontrados: ");
    ...
    private void remover() {
        if (bloco.getQuantidadeDeNotas() == 0) {
            sln("não existe notas cadastradas");
            return;
        }
        ...
        try {
            ...
            if (notaRemovida != null) {
                sln("a nota " + notaRemovida.getTitulo() + " foi removida!");
            } else {
                sln(" não existe uma nota com o index " + indice);
            }
        } catch (NumberFormatException e) {
            sln("excessão " + e);
            sln(indiceS + " não é um numero");
        }
    }

    private void listaParaRemocao() {
        sln("-----------------------------------------");
    ...
    private void mostrarNotas(ArrayList<Nota> notas) {
        ...
        sln("-----------------------------------------");
        sln(notas.size() + "  encontrada(s)");
        sln("-----------------------------------------");

        int i = 0;
        for (Nota nota : notas) { //acessando a colecction inteira
            sln("Nota: " + ++i);
            sln("Título: \n\t" + nota.getTitulo());
            sln("Texto: \n\t" + nota.getTexto());
            sln("-----------------------------------------");
