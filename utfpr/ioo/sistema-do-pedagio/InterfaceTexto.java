...
public class InterfaceTexto {
	...
	private void registrarPassagemDeVeiculo() {
		...
		String hora = leia("Digite o horário da passagem: hora:minutos:segundos");
		String data = leia("Digite a data da passasgem: dia/mês/ano ");

		switch(tipoDoVeiculo) {
			case "1":
				pedagio.registrarPassagemVeiculo(new Automovel(placa,hora,data));
				break;
			case "2":
				int numeroDeEixos = leiaInteiro("Digite o numero de eixos: ");
				pedagio.registrarPassagemVeiculo(new Caminhao(placa, numeroDeEixos,hora,data));
				break;
		}
	}
	...
	private void listarVeiculos() {
		...
		sln("---------------------------------------------------------------------");
		sln("| Placa  |    Tipo    | Nº de eixos | Tipo de Tarifa |  Horário  |  Data  |  Tarifa Total |");
		sln("---------------------------------------------------------------------");

		for (Veiculo v : pedagio.getVeiculos()) {
			sf("| %-6s | %-10s | %-11d | %-14s | %-12s || %-6s || %-10s |\\n", v.getPlaca(),
															v.getTipo(),
												  			v.getNumeroDeEixos(),
												  			v.getTipoDeTarifa(),
												  			v.getHora(),
